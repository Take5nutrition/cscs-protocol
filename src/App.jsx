...
const CURRICULUM = {
  ...
  phase22: {
    id: 'phase22',
    title: 'Phase XXII — Cardiorespiratory Programming',
    subtitle: 'VO₂max, HR zones, endurance methods, altitude',
    color: '#0096C7',
    domains: [
      {
        id: 'vo2max-testing',
        title: 'VO₂max Testing & Interpretation',
        cards: [
          { q: 'What is VO₂max and what factors limit it?', a: 'Maximum rate of oxygen consumption during maximal exercise. Primary limiters: cardiac output (HR × SV), oxygen-carrying capacity (hemoglobin), and muscle oxidative capacity. VO₂max = Q × (a-vO₂ diff). Elite endurance athletes: 70-85 mL/kg/min. Untrained: 35-45. Central (cardiac) factors limit most athletes; peripheral (muscle) factors can be a secondary limiter in well-trained individuals.', tags: ['cardio', 'vo2max', 'testing', 'high-yield'] },
          { q: 'What are the main field and lab tests for VO₂max estimation?', a: 'Lab (gold standard): graded exercise test (GXT) with metabolic cart — maximal or submaximal. Field estimates: Cooper 12-min run (VO₂max ≈ (d-504.9)/44.73), 1.5-mile run, beep test (20m shuttle). Submaximal lab: Astrand-Rhyming cycle ergometer, YMCA cycle protocol — estimate from HR response. Key: true VO₂max requires maximal effort; field tests estimate via regression equations with ±10-15% error.', tags: ['cardio', 'vo2max', 'testing', 'high-yield'] },
          { q: 'What is the Fick equation and how is it applied?', a: 'VO₂ = Q × (a-vO₂ difference). Q = cardiac output (HR × stroke volume). a-vO₂ diff = oxygen extracted by tissues (arterial O₂ – venous O₂). At rest: a-vO₂ diff ≈ 5 mL O₂/100 mL blood. At maximal exercise: ≈ 15-16 mL O₂/100 mL blood. Training increases VO₂max primarily by increasing SV (larger Q) and muscle oxidative enzyme activity (larger a-vO₂ diff).', tags: ['cardio', 'fick', 'vo2max', 'high-yield'] },
          { q: 'What is the difference between VO₂max and VO₂ reserve (VO₂R)?', a: 'VO₂max: absolute ceiling of oxygen consumption. VO₂R: VO₂max minus resting VO₂ (≈3.5 mL/kg/min). Training intensity using VO₂R parallels HRR method: Target VO₂ = (VO₂max – VO₂rest) × intensity% + VO₂rest. The NSCA prefers VO₂R-based prescription for precision because it accounts for individual resting metabolism. %VO₂R ≈ %HRR ≈ %maximal aerobic capacity.', tags: ['cardio', 'vo2max', 'prescription', 'high-yield'] },
          { q: 'What are the criteria confirming a true VO₂max has been reached?', a: 'Primary: plateau in VO₂ despite increasing workload (<150 mL/min or <2.1 mL/kg/min increase). Secondary: RER ≥1.10 (CO₂ output exceeds O₂ intake — lactate buffering), HR within 10 bpm of age-predicted max, blood lactate >8 mmol/L, RPE ≥17-18 (Borg 6-20). True plateau occurs in <50% of subjects — secondary criteria confirm maximal effort when plateau absent.', tags: ['cardio', 'vo2max', 'testing', 'high-yield'] },
          { q: 'What does the lactate threshold represent and why does it matter for programming?', a: 'Lactate threshold (LT1): intensity where blood lactate first rises above resting (~2 mmol/L). Lactate threshold 2 / OBLA: intensity where lactate accumulates rapidly (~4 mmol/L). LT is the best predictor of endurance performance (better than VO₂max in trained populations). Training at or near LT2 produces the greatest improvements in lactate clearance and mitochondrial density. LT typically occurs at 50-70% VO₂max in untrained, 80-90% in elite athletes.', tags: ['cardio', 'lactate', 'threshold', 'programming', 'high-yield'] },
        ]
      },
      {
        id: 'hr-zones',
        ...
        cards: [
          { q: 'What are the two main methods for prescribing aerobic training intensity?', a: '%MHR method: Target HR = MHR × intensity%. Simple but less accurate — MHR = 220 – age (±10-12 bpm). Heart Rate Reserve (HRR / Karvonen method): Target HR = [(MHR – RHR) × intensity%] + RHR. HRR is preferred because it accounts for individual resting HR. %HRR correlates closely with %VO₂R and is more precise at low-moderate intensities.', tags: ['cardio', 'hr-zones', 'prescription', 'high-yield'] },
          { q: 'What are the ACSM/NSCA aerobic training intensity zones?', a: 'Very light: <57% MHR / <37% HRR. Light: 57-63% / 37-45%. Moderate: 64-76% / 46-63%. Vigorous: 77-95% / 64-90%. Near-maximal/maximal: ≥96% / ≥91%. NSCA training zones roughly: Zone 1 (base/recovery): 60-70% MHR. Zone 2 (aerobic threshold): 70-80%. Zone 3 (LT2/tempo): 80-90%. Zone 4 (VO₂max intervals): 90-100%. Zone 5 (supramaximal): >100% VO₂max via sprint intervals.', tags: ['cardio', 'hr-zones', 'intensity', 'high-yield'] },
          { q: 'How do you calculate a training heart rate using the Karvonen method?', a: 'Target HR = [(MHR – RHR) × intensity%] + RHR. Example: athlete, MHR = 190, RHR = 55, goal = 70% HRR. Target HR = [(190-55) × 0.70] + 55 = [135 × 0.70] + 55 = 94.5 + 55 = 150 bpm. At 70% HRR this athlete trains at 150 bpm — much higher than 70% MHR (133 bpm) because a fit athlete has high SV and a low RHR.', tags: ['cardio', 'karvonen', 'formula', 'high-yield'] },
          { q: 'What is HIIT and how does it differ from MICT for cardiorespiratory adaptation?', a: 'HIIT (High-Intensity Interval Training): alternating bouts at ≥90% MHR (often 100%+ VO₂max) with recovery periods. MICT (Moderate Intensity Continuous Training): sustained 64-76% MHR for 30-60 min. HIIT produces greater improvements in VO₂max per unit time, better central cardiac adaptations (increased SV), and superior metabolic adaptations. MICT is better for volume-sensitive adaptations (fat oxidation, capillary density). NSCA: both are valid; HIIT is time-efficient but requires higher recovery.', tags: ['cardio', 'hiit', 'mict', 'programming', 'high-yield'] },
          { q: 'What is the FITT principle applied to cardiorespiratory training?', a: 'Frequency: 3-5 days/week for moderate intensity; 3 days/week sufficient for vigorous. Intensity: 64-95% MHR or 46-90% HRR for meaningful adaptation. Time: 20-60 min continuous or accumulated; HIIT 20-30 min of interval work. Type: rhythmic, large-muscle group activity (running, cycling, rowing). NSCA adds Mode (Type) and Volume considerations. Minimum for VO₂max improvement: 3×/week at 60-80% MHR for 20+ min.', tags: ['cardio', 'fitt', 'programming', 'high-yield'] },
          { q: 'How do you determine appropriate work:rest ratios for aerobic interval training?', a: 'Depends on the energy system targeted. VO₂max intervals (90-100% VO₂max): work 3-5 min, rest 1:1 ratio (e.g., 4 min on, 4 min off). Lactate threshold: continuous or 10-20 min at LT2. Anaerobic intervals (>100% VO₂max): 30-60 sec work, 1:3-5 rest ratio. Principle: shorter, more intense → longer relative rest to allow adequate recovery and maintain quality. NSCA tables: base endurance training uses 1:1 to 1:3 depending on goal.', tags: ['cardio', 'intervals', 'work-rest', 'programming', 'high-yield'] },
        ]
      },
      {
        id: 'endurance-methods',
        title: 'Endurance Training Methods',
        cards: [
          { q: 'What is polarized training and what does the research support?', a: 'Training distribution where ~80% of volume is performed at low intensity (Zone 1, below LT1) and ~20% at high intensity (Zone 3-4, near VO₂max), with very little moderate-intensity work. Research in elite endurance athletes (Seiler) shows polarized approach produces greater VO₂max gains and performance improvements than "threshold-dominant" approaches. Rationale: low intensity builds aerobic base without fatigue; high intensity provides potent stimulus; moderate is "junk mileage" — enough stress to fatigue, not enough to drive adaptation.', tags: ['cardio', 'polarized', 'endurance', 'programming'] },
          { q: 'What is concurrent training interference and how do you minimize it?', a: 'The "interference effect": combining endurance and strength training can blunt hypertrophy and maximal strength gains (but not power, to same degree). Mechanism: AMPK activation from endurance suppresses mTOR signaling for muscle protein synthesis. Minimize by: (1) separating sessions by 6-8+ hours, (2) placing strength before endurance when done in same session, (3) reducing endurance volume and intensity during hypertrophy phases, (4) using cycling/rowing instead of running (less eccentric muscle damage).', tags: ['cardio', 'concurrent', 'interference', 'programming', 'high-yield'] },
          { q: 'What cardiorespiratory adaptations occur with endurance training?', a: 'Central: increased SV (eccentric cardiac hypertrophy, increased blood volume), decreased resting HR, increased maximal Q, increased plasma volume (+10-12% in weeks). Peripheral: increased capillary density, increased mitochondrial density and size, increased oxidative enzymes (CS, SDH), increased myoglobin, improved fat oxidation efficiency. Acute: increased HR, SV, Q, VE, a-vO₂ diff. Chronic: reduced HR at same absolute workload (most easily measured adaptation).', tags: ['cardio', 'adaptations', 'endurance', 'high-yield'] },
          { q: 'How do you program cardiorespiratory training for a team sport athlete vs. an endurance athlete?', a: 'Team sport athlete: prioritize alactic power (sprint repeats), lactic capacity (repeated sprint ability), aerobic base for recovery between efforts. Use sport-specific work:rest ratios mimicking game demands. Avoid chronic high-volume endurance that interferes with power. Endurance athlete: progressive overload of volume first, then intensity; periodize with high volume base → threshold development → VO₂max intervals → racing peaks. NSCA: needs analysis must identify dominant energy systems and movement patterns for each sport.', tags: ['cardio', 'team-sport', 'programming', 'needs-analysis', 'high-yield'] },
        ]
      },
      {
        id: 'altitude-heat',
        title: 'Altitude & Environmental Physiology',
        cards: [
          { q: 'What is the Live High Train Low (LHTL) altitude training model and why is it preferred?', a: 'Sleeping/resting at altitude (2,000-2,500m) to stimulate EPO release and RBC production while training at sea level to maintain training quality (VO₂max and speed are reduced at altitude). EPO response peaks at 2,000-2,800m — higher altitudes do not produce proportionally more EPO. Duration: minimum 3-4 weeks needed for meaningful hematological changes. Benefits: 3-5% increase in RBC mass and VO₂max. Traditional "live high train high" reduces training quality too much.', tags: ['altitude', 'lhtl', 'endurance', 'environmental', 'high-yield'] },
          { q: 'What are the key physiological responses to acute altitude exposure?', a: 'Immediate: hyperventilation (hypoxic ventilatory response), decreased SaO₂, decreased VO₂max (~1% per 100m above 1500m), increased HR at same absolute workload. Short-term (days): increased EPO secretion (kidney senses hypoxia via HIF-1α), plasma volume contraction (hemoconcentration — elevated Hct without true increase in RBCs). Long-term (weeks): increased RBC production, increased 2,3-DPG (right-shifts O₂ dissociation curve), increased capillary density. Altitude sickness: headache, nausea, fatigue at >2,400m without acclimatization.', tags: ['altitude', 'physiology', 'environmental', 'high-yield'] },
          { q: 'How does heat affect exercise performance and what are the thermoregulatory mechanisms?', a: 'Heat increases cardiovascular strain: more blood diverted to skin for cooling → reduced muscle blood flow. Core temp >40°C impairs CNS function and force production. Mechanisms: sweating (primary), cutaneous vasodilation, radiation, convection. Heat acclimatization (10-14 days): increased plasma volume, earlier sweating onset, lower sweat sodium concentration, reduced core temp at same workload. Performance restored after 10-14 days heat adaptation. Dehydration of 2% body mass reduces aerobic performance 10-20%.', tags: ['heat', 'thermoregulation', 'environmental', 'high-yield'] },
          { q: 'What is the heat illness spectrum and what triggers each stage?', a: 'Heat cramps: muscle spasms from electrolyte loss (sodium depletion) — replace fluids and sodium. Heat syncope: dizziness/fainting from peripheral vasodilation and postural pooling — position supine, cool, hydrate. Heat exhaustion: core temp 38-40°C, profuse sweating, weakness, nausea — stop exercise, cool aggressively, IV fluids if severe. Exertional heat stroke (EHS): core temp >40°C + CNS dysfunction (confusion, ataxia, loss of consciousness) — TRUE EMERGENCY. Immediate whole-body cold water immersion (WBCWI) is gold standard treatment. EHS without rapid cooling is fatal.', tags: ['heat', 'illness', 'safety', 'environmental', 'high-yield'] },
        ]
      },
    ]
  },
  ...
  phase23: {
    id: 'phase23',
    title: 'Phase XXIII — Recovery, Monitoring & Readiness',
    subtitle: 'Overtraining, detraining, taper, HRV, blood markers',
    color: '#AE2012',
    domains: [
      {
        id: 'overtraining',
        title: 'Overtraining & Overreaching',
        cards: [
          { q: 'What are the three stages of the overtraining continuum?', a: 'Functional Overreaching (FOR): planned short-term performance decrement with full recovery in days-weeks; leads to supercompensation. Non-Functional Overreaching (NFOR): unplanned, prolonged performance decrement; recovery takes weeks to months; early systemic symptoms. Overtraining Syndrome (OTS): full syndrome with documented performance decline, mood disturbances, hormonal disruption; recovery requires months to a year+. Key distinction: OTS requires ruling out other causes (illness, depression, RED-S). OTS is a diagnosis of exclusion.', tags: ['overtraining', 'recovery', 'monitoring', 'high-yield'] },
          { q: 'What are the signs and symptoms of overtraining syndrome?', a: 'Performance: prolonged decline in strength, speed, power; increased effort at standard workloads; inability to complete normal training. Physiological: elevated resting HR, suppressed HRV, elevated cortisol, decreased testosterone:cortisol ratio, disrupted sleep. Psychological: mood disturbances (POMS: increased fatigue/anger/depression, decreased vigor), loss of motivation, increased perceived exertion. Immunological: frequent illness, elevated inflammatory markers. NSCA: no single biomarker definitively diagnoses OTS — it is a clinical diagnosis based on pattern of symptoms + exclusion of other causes.', tags: ['overtraining', 'symptoms', 'monitoring', 'high-yield'] },
          { q: 'What is the testosterone:cortisol (T:C) ratio and what does a change indicate?', a: 'Ratio of serum testosterone to cortisol — reflects anabolic:catabolic balance. T:C ratio >0.35 generally considered anabolic. A decline of >30% from baseline T:C ratio is associated with overreaching and inadequate recovery. Cortisol rises with training stress and psychological stress; testosterone suppressed by chronic overload. Limitation: high inter-individual variability — individual baseline matters more than population norms. Best used longitudinally (tracking same athlete over time), not cross-sectionally.', tags: ['overtraining', 'hormones', 'monitoring', 'high-yield'] },
          { q: 'What is Relative Energy Deficiency in Sport (RED-S) and how does it overlap with overtraining?', a: 'RED-S (formerly Female Athlete Triad): insufficient energy availability relative to exercise expenditure — affects hormonal function, bone health, immune function, cardiovascular health, and psychological wellbeing. Energy availability <30 kcal/kg FFM/day is problematic. Symptoms mimic OTS: fatigue, performance decline, stress fractures, amenorrhea (females), decreased testosterone (males). Critical distinction: RED-S is driven by energy deficit, not training load. Fixing OTS without addressing energy intake in RED-S athletes will not resolve the syndrome.', tags: ['overtraining', 'RED-S', 'nutrition', 'special-populations', 'high-yield'] },
          { q: 'How do you monitor training load to prevent overtraining?', a: 'Internal load: RPE-based session-RPE (sRPE = RPE × duration); HRV; resting HR. External load: GPS distance/speed, barbell volume (sets × reps × load). Acute:Chronic Workload Ratio (ACWR): acute load (7-day) / chronic load (28-day rolling avg). "Sweet spot": 0.8-1.3. >1.5 = elevated injury risk (spike ratio). Wellness surveys: POMS, Hooper Index (sleep, fatigue, stress, muscle soreness). Best practice: combine subjective (wellness) + objective (HRV, performance testing) + external load metrics weekly.', tags: ['overtraining', 'monitoring', 'load-management', 'high-yield'] },
        ]
      },
      {
        id: 'detraining',
        title: 'Detraining Timelines',
        cards: [
          { q: 'How quickly do aerobic adaptations detrain and what is maintained longest?', a: 'VO₂max decreases 4-14% in first 3-4 weeks; 50% of aerobic gains lost in 4-8 weeks of complete inactivity. Sequence of loss: plasma volume (fastest — lost in days), peripheral adaptations (capillary density, oxidative enzymes — weeks), central cardiac adaptations (SV, ventricular size — months). Highly trained athletes lose fitness faster in absolute terms but retain more in relative terms. Even 2 sessions/week at reduced volume but maintained intensity preserves 90%+ of VO₂max for up to 12 weeks.', tags: ['detraining', 'aerobic', 'maintenance', 'high-yield'] },
          { q: 'How quickly do strength and muscle mass detrain?', a: 'Neural adaptations (strength without hypertrophy): lost first — force production decreases within 1-2 weeks. Hypertrophy: slower to regress — cross-sectional area preserved 4-8 weeks before significant atrophy. After 6+ months: return toward pre-training baseline. Rate varies: trained athletes detrain slower than untrained due to myonuclei retention ("muscle memory" — myonuclei persist, enabling faster re-training). Maintenance: 1 session/week at full intensity preserves most strength for 6-12 weeks; 2 sets per muscle group sufficient if intensity maintained.', tags: ['detraining', 'strength', 'muscle', 'maintenance', 'high-yield'] },
          { q: 'What is the minimal dose of training needed to maintain fitness during reduced training?', a: 'Frequency can be reduced most aggressively. Volume can be cut 33-67% while maintaining intensity. Intensity must be maintained — it is the critical variable. NSCA guidance: strength: 1×/week, 1 set at original training intensity maintains 1RM for 12 weeks. Endurance: 2×/week, reduced volume but maintained VO₂max-level intensity maintains aerobic fitness 6-12 weeks. Power: frequency most critical — 2×/week minimum to maintain RFD. Key principle: you can survive on low dose IF intensity is not compromised.', tags: ['detraining', 'maintenance', 'minimal-dose', 'high-yield'] },
          { q: 'What is "muscle memory" at the cellular level?', a: 'Myonuclei acquired during hypertrophy training are retained even after muscle atrophy from detraining. Nuclei do not appear to be lost during atrophy. When training resumes, these retained myonuclei enable faster protein synthesis and re-hypertrophy — hence muscle grows back faster in previously trained individuals than in naive trainees. Time course of myonuclear retention: likely permanent or at minimum persists for months to years. Practical implication: a previously trained athlete who took years off will still regain muscle faster than someone who never trained.', tags: ['detraining', 'muscle-memory', 'myonuclei', 'hypertrophy'] },
        ]
      },
      {
        id: 'tapering',
        title: 'Tapering Protocols',
        cards: [
          { q: 'What is a taper and what are the main types?', a: 'Planned reduction in training load (primarily volume) in the days-weeks before competition to allow full recovery while maintaining fitness. Types: linear (gradual steady reduction), step (abrupt volume drops), exponential with slow decay (~13% reduction/day), exponential with fast decay (~25%/day). Research: exponential taper with fast decay produces greatest performance improvements. Duration: 4-28 days — most research supports 7-14 days for most sports. Strength/power sports: shorter taper (3-7 days). Endurance: longer (10-21 days).', tags: ['taper', 'competition', 'programming', 'high-yield'] },
          { q: 'What variables change during a taper and which is most critical to maintain?', a: 'Volume: reduced 41-60% from peak training. Frequency: maintained or reduced by at most 20-50%. Intensity: MAINTAINED or slightly increased. NSCA position: intensity is the most critical variable to preserve during taper — reducing intensity causes rapid detraining of specific adaptations. Reducing volume alone, with maintained intensity and frequency, produces 2-3% performance improvement. Reducing all three causes detraining. Common mistake: athletes and coaches cut intensity during taper thinking it equals recovery — this is wrong.', tags: ['taper', 'intensity', 'volume', 'programming', 'high-yield'] },
          { q: 'What physiological mechanisms explain performance gains from tapering?', a: 'Muscle glycogen restoration (super-compensation): 15-20% increase with reduced training + adequate CHO. Muscle damage repair: decreased CK, LDH; reduced soreness. Neural: improved motor unit recruitment, firing rate, synchronization. Hormonal: testosterone:cortisol ratio restoration toward anabolic. Hematological: RBC mass maintained while plasma volume normalizes → increased [Hgb]. Psychological: decreased fatigue (POMS vigor increases, fatigue decreases). Combined: 0.5-6% performance improvement — meaningful in elite sport where margins are tiny.', tags: ['taper', 'physiology', 'programming', 'high-yield'] },
          { q: 'How do you structure the final week before a major competition?', a: 'Days 7-5: maintain 1-2 high-intensity sessions (full speed/power work at competition intensity). Reduce volume 50-60%. Days 4-3: light technical work, activation exercises, movement quality. Reduce volume 70-80%. Days 2-1: very light activity (10-20 min), sport-specific movement, no heavy loading. Day of competition: dynamic warm-up 90-120 min before event. Nutrition: CHO loading days 2-3 out for endurance. NSCA: exact structure depends on sport — contact/collision sports need longer last heavy day to allow recovery from soreness.', tags: ['taper', 'competition-prep', 'weekly-structure', 'high-yield'] },
        ]
      },
      {
        id: 'hrv-monitoring',
        ...
        cards: [
          { q: 'What is Heart Rate Variability (HRV) and what does it reflect?', a: 'Variation in time between successive heartbeats (R-R intervals). Measured in ms. High HRV = high parasympathetic dominance = good recovery/readiness. Low HRV = sympathetic dominance = stress, fatigue, insufficient recovery. HRV is regulated by the autonomic nervous system — fluctuates with training load, sleep quality, psychological stress, illness, alcohol, and nutrition. Best measured first thing in the morning (supine, consistent protocol). Population norms meaningless — individual baseline tracking is what matters.', tags: ['hrv', 'monitoring', 'recovery', 'readiness', 'high-yield'] },
          { q: 'How should coaches use HRV data in training decisions?', a: 'Track individual baseline over 4-6 weeks. Daily decision framework: HRV within ±0.5 CV of baseline → proceed as planned. HRV 1-2 SD below normal → modify session (reduce volume/intensity, prioritize technique). HRV >2 SD below normal → active recovery only or rest. Green/amber/red traffic light is a practical coaching tool. HRV trends matter more than single readings — a week of suppressed HRV signals accumulated fatigue. HRV should be combined with wellness surveys, not used in isolation. Apps: HRV4Training, Elite HRV.', tags: ['hrv', 'monitoring', 'readiness', 'coaching', 'high-yield'] },
          { q: 'What blood markers are useful for monitoring athlete readiness and what do they indicate?', a: 'Ferritin: iron stores — low ferritin causes fatigue and decreased VO₂max even before anemia develops; target >30 ng/mL (athletes may need >50). CBC: hemoglobin, hematocrit — anemia impairs O₂ transport. T:C ratio: anabolic:catabolic balance (see above). CK (creatine kinase): muscle damage marker — elevated 24-72 hrs post heavy training; sustained elevation = inadequate recovery. Cortisol: chronic elevation = overreaching. Vitamin D: deficiency common in indoor athletes; affects muscle function and immune health. Note: coaches need medical collaboration for bloodwork — it is outside typical coaching scope.', tags: ['blood-markers', 'monitoring', 'readiness', 'high-yield'] },
          { q: 'What is the Acute:Chronic Workload Ratio (ACWR) and how is it used?', a: 'ACWR = Acute load (7-day rolling sum) ÷ Chronic load (28-day rolling average). Represents training "fitness" relative to recent "fatigue." Sweet spot: 0.8-1.3. <0.8: undertraining — may lose fitness. 1.3-1.5: elevated risk zone — monitor closely. >1.5: spike zone — significantly elevated injury risk. Used in team sports (AFL, rugby, soccer) with GPS load data (distance, high-speed running). Controversy: ACWR has limitations as sole injury predictor — athlete load tolerance, sleep, illness confound it. Use as one monitoring tool among many.', tags: ['acwr', 'load-management', 'monitoring', 'injury-prevention', 'high-yield'] },
        ]
      },
    ]
  },
  ...
  phase24: {
    id: 'phase24',
    title: 'Phase XXIV — Advanced Training Methods',
    subtitle: 'VBT, PAP, BFR, structural balance, tendon science',
    color: '#6A4C93',
    domains: [
      {
        id: 'vbt',
        ...
        cards: [
          { q: 'What is Velocity-Based Training (VBT) and what does it autoregulate?', a: 'Training approach using real-time mean concentric velocity (MCV) to prescribe load and manage fatigue. Tools: linear position transducers (GymAware), accelerometers (PUSH Band). Replaces fixed % 1RM with velocity zones — accounts for daily variation in readiness. % velocity loss as stopping criterion: stop set when velocity drops >20% from rep 1 (strength-power work) or >40% (hypertrophy). Key advantage: accounts for day-to-day variation in 1RM (can vary 5-10%) — a fixed 80% 1RM may be 73% or 87% depending on readiness.', tags: ['vbt', 'advanced-methods', 'autoregulation', 'high-yield'] },
          { q: 'What are the general velocity zones for different training qualities?', a: 'Maximal strength (1RM zone): <0.35 m/s. Strength: 0.35-0.55 m/s (85-100% 1RM approx). Strength-speed: 0.55-0.75 m/s (~70-85% 1RM). Power / speed-strength: 0.75-1.0 m/s (~50-70% 1RM). Speed: >1.0 m/s (<50% 1RM). Note: velocities are exercise-specific — squat velocities differ from bench press. Load-velocity profiles are individual and must be established per athlete per exercise. Train the quality by targeting the velocity zone, not just the percentage.', tags: ['vbt', 'velocity-zones', 'programming', 'high-yield'] },
          { q: 'What is the load-velocity relationship and how do you build an individual profile?', a: 'For most exercises, a strong linear relationship exists between % 1RM and mean concentric velocity. Individual profiles built by: (1) perform multiple sets across range of loads (40%, 60%, 70%, 80%, 90% estimated 1RM), (2) measure mean velocity at each load, (3) fit linear regression, (4) extrapolate 1RM from minimum velocity threshold (MVT, typically 0.15-0.30 m/s for squat). MVT is highly individual and exercise-specific. Profile is then used to auto-estimate 1RM daily from a single warm-up set velocity.', tags: ['vbt', 'load-velocity', '1rm-estimation', 'programming'] },
          { q: 'What is velocity loss and how is it used to regulate volume?', a: 'Velocity loss = (V_rep1 - V_current) / V_rep1 × 100%. Used as stopping criterion per set instead of fixed rep count. Research (Pareja-Blanco): 20% velocity loss = strength/power focus (lower fatigue, more reps across session). 40% velocity loss = hypertrophy/strength endurance (more mechanical work per set, greater fatigue). Advantage: naturally adjusts reps per set based on daily readiness. Fatigued athlete stops sooner (maintains quality); fresh athlete does more reps before dropout velocity.', tags: ['vbt', 'velocity-loss', 'autoregulation', 'programming', 'high-yield'] },
        ]
      },
      {
        id: 'pap',
        ...
        cards: [
          { q: 'What is Post-Activation Potentiation (PAP) and what is its mechanism?', a: 'Acute enhancement of muscle contractile properties following a heavy conditioning activity (CA). Mechanism: phosphorylation of myosin regulatory light chains (RLC) by myosin light chain kinase (MLCK) — increases actin-myosin cross-bridge sensitivity to Ca²⁺, improving rate of force development. Result: improved power output in subsequent explosive activity (sprint, jump, throw). PAP co-exists with fatigue — the net effect depends on the balance between potentiation and fatigue.', tags: ['pap', 'advanced-methods', 'physiology', 'power', 'high-yield'] },
          { q: 'What are the optimal conditions for PAP to enhance performance?', a: 'Conditioning activity: 1-5 RM squat, deadlift, or sport-specific heavy movement at 85-95% 1RM (3-5 reps). Rest interval: 4-12 minutes (well-trained: 4-8 min; less trained: 8-12 min). Too little rest = fatigue dominates. Too much = potentiation dissipates. Training status matters: stronger athletes (≥1.5× BW squat) show greater PAP response. Post-CA explosive activity: sprint, countermovement jump, plyometric, or throw. Complex training and contrast training use PAP systematically.', tags: ['pap', 'complex-training', 'programming', 'power', 'high-yield'] },
          { q: 'What is complex training and how does it differ from contrast training?', a: 'Complex training: pairs a heavy compound lift with a biomechanically similar explosive exercise in the same set/superset (e.g., 3 RM squat → box jump). Exploits PAP to enhance explosive output. Contrast training: alternates heavy and light loads within a training session (e.g., heavy week → lighter speed week, or heavy set → speed set with rest between). Both use PAP mechanism but differ in structure. Complex training is intra-session PAP; contrast training can be inter-session. NSCA recommends complex training for advanced athletes seeking power development.', tags: ['pap', 'complex-training', 'contrast-training', 'programming', 'high-yield'] },
          { q: 'What warm-up strategies use PAP principles to enhance acute performance?', a: 'Heavy warm-up sets: 1-3 reps at 80-90% before competition lift (4-8 min rest). Loaded stretches: heavy hip flexor stretch before sprinting — increases subsequent hip extension power. Weighted vest sprint into unweighted sprint: resisted/assisted contrast. Drop-jump before vertical jump test: plyometric PAP. Key for coaches: PAP warm-up protocols should be individualized — training level, fiber type (type II dominant respond more), and rest interval all interact. Test in training before implementing competition-day.', tags: ['pap', 'warm-up', 'performance', 'competition', 'high-yield'] },
        ]
      },
      {
        id: 'bfr',
        ...
        cards: [
          { q: 'What is Blood Flow Restriction (BFR) training and what are its mechanisms?', a: 'Application of a pneumatic cuff to a proximal limb to partially restrict venous outflow (and partially arterial inflow) during low-load resistance exercise. Loads: 20-40% 1RM (vs typical 60-80% for hypertrophy). Mechanisms: (1) metabolite accumulation (lactate, H⁺) in restricted limb → mTOR signaling, growth hormone release; (2) hypoxia → preferential type II fiber recruitment (fatigue of type I from restriction); (3) mechanical tension on type II fibers. Result: hypertrophy and strength gains comparable to heavy training at much lower loads.', tags: ['bfr', 'advanced-methods', 'rehabilitation', 'hypertrophy', 'high-yield'] },
          { q: 'What are the clinical applications and populations best suited for BFR?', a: 'Post-surgical rehab: patients unable to load heavily (ACL reconstruction, rotator cuff repair, arthroplasty) can maintain/build muscle at loads that don\'t stress healing tissue. Elderly: low loads reduce joint stress while preserving anabolic stimulus. Injury prevention: athletes in-season who need to reduce joint loading but maintain hypertrophy. Distance athletes: supplemental muscle stimulus without eccentric damage that interferes with running. Contraindications: deep vein thrombosis history, sickle cell trait, peripheral vascular disease, severe hypertension, open wounds at cuff site.', tags: ['bfr', 'rehabilitation', 'special-populations', 'high-yield'] },
          { q: 'What are the BFR protocol parameters?', a: 'Cuff pressure: typically 40-80% of limb occlusion pressure (LOP). Individualized to limb circumference — wider cuffs need less pressure than narrow cuffs. Upper limb: 50-70% LOP. Lower limb: 60-80% LOP. Load: 20-40% 1RM. Rep scheme: 4 sets, 30-15-15-15 reps (or continuous until failure). Rest between sets: 30-60 seconds (short to maintain metabolite accumulation). Session length: 15-20 min with cuff applied. Frequency: 2-3×/week. Acute discomfort and DOMS are common but serious adverse events are rare when protocols are followed.', tags: ['bfr', 'protocols', 'parameters', 'high-yield'] },
        ]
      },
      {
        id: 'structural-balance',
        title: 'Structural Balance & Ratios',
        cards: [
          { q: 'What is structural balance and why does it matter for injury prevention?', a: 'Appropriate strength ratios between agonist-antagonist muscle groups and between bilateral limbs. Imbalances increase injury risk by creating aberrant joint forces and altering movement patterns. Most tested ratios: hamstring:quadriceps (H:Q) = 0.60-0.80 (isometric) or functional H:Q >0.61 (hamstring eccentric:quad concentric). Bilateral symmetry: >85-90% symmetry on single-leg tests (e.g., single-leg squat, hop test) before return to play. External:internal rotator ratio (shoulder): 0.66-0.75. Identifying and correcting imbalances is a core function of the S&C coach.', tags: ['structural-balance', 'injury-prevention', 'ratios', 'high-yield'] },
          { q: 'What is the hamstring:quadricep (H:Q) ratio and what does a deficit indicate?', a: 'Conventional H:Q ratio: peak hamstring torque ÷ peak quad torque on isokinetic dynamometer at 60°/s. Normal: 0.60-0.80. Functional H:Q: eccentric hamstring torque ÷ concentric quad torque at 240°/s — better predictor of hamstring injury. H:Q <0.60: elevated hamstring strain risk. Between-limb asymmetry >15% is a red flag. Testing: Biodex isokinetic dynamometer (gold standard). Field proxy: Nordic hamstring curl strength. Correction: eccentric-focused hamstring work (Nordic curls, RDLs, glute-ham raises), prioritized in program.', tags: ['structural-balance', 'hamstring', 'quad', 'injury-prevention', 'high-yield'] },
          { q: 'What are common bilateral strength asymmetries to monitor and what thresholds trigger concern?', a: 'Threshold: >10-15% asymmetry on most tests is clinically significant. Key tests: Single-leg squat jump (peak power asymmetry). Single-leg hop for distance (distance asymmetry). Isometric mid-thigh pull (force asymmetry). Return to sport criteria post-ACL: limb symmetry index (LSI) ≥90% on functional tests before full clearance. Chronic asymmetries: dominant side typically stronger in rotation sport athletes — not always pathological. Acute changes from baseline are more concerning than chronic stable asymmetry.', tags: ['structural-balance', 'asymmetry', 'return-to-sport', 'monitoring', 'high-yield'] },
          { q: 'What is the Shoulder External:Internal Rotation ratio and its implications for overhead athletes?', a: 'ER:IR ratio (isokinetic): 0.66-0.75 considered normal. Overhead athletes (baseball pitchers, swimmers, volleyball): often have reduced ER:IR due to increased internal rotation strength and adaptive humeral retroversion. ER weakness relative to IR → elevated rotator cuff injury risk. Glenohumeral Internal Rotation Deficit (GIRD): loss of IR > 20° compared to non-dominant — associated with labral and cuff pathology. Correction: posterior capsule stretching (sleeper stretch), ER strengthening with light cables/bands. Prevention > correction.', tags: ['structural-balance', 'shoulder', 'overhead', 'injury-prevention', 'high-yield'] },
        ]
      },
      {
        id: 'tendon-physiology',
        title: 'Tendon & Connective Tissue Science',
        cards: [
          { q: 'How do tendons adapt to training and why is their adaptation rate important?', a: 'Tendons adapt primarily by increasing collagen content and cross-link density — resulting in increased stiffness and tensile strength. Adaptation rate is significantly slower than muscle: muscle can increase force capacity within weeks; tendon structural adaptation lags 8-12+ weeks. This mismatch creates injury risk during rapid loading increases — muscle can generate more force than the tendon can yet tolerate. Practical implication: progress tendon-loading exercises conservatively, especially in returning athletes and young athletes in growth spurts.', tags: ['tendon', 'connective-tissue', 'adaptation', 'injury-prevention', 'high-yield'] },
          { q: 'What is the tendon collagen synthesis response to exercise?', a: 'Collagen synthesis peaks 24 hours after loading; returns to baseline by 72 hours. Loading type matters: tensile loading (eccentric exercise) most effective for stimulating collagen synthesis. Isometric loading: effective and less painful — used in tendinopathy management. Nutritional support: collagen peptide + vitamin C supplementation 60 min before tendon-loading exercise doubles collagen synthesis markers (Shaw et al. research). Load magnitude and rate both matter — tendons respond to strain magnitude and loading rate (strain rate).', tags: ['tendon', 'collagen', 'synthesis', 'nutrition', 'high-yield'] },
          { q: 'What is tendinopathy and how does it differ from tendon rupture?', a: 'Tendinopathy: chronic, degenerative condition from repeated overload without adequate recovery — characterized by pain, thickening, disorganized collagen structure (tendinosis). Not an inflammatory condition (hence not "tendinitis"). Common sites: patellar (jumper\'s knee), Achilles, proximal hamstring, rotator cuff. Management: heavy slow resistance training (HSR) — progressive eccentric loading, pain monitoring (acceptable if <5/10 and returns to baseline). Tendon rupture: complete or partial structural failure — typically requires surgical intervention and months of rehabilitation.', tags: ['tendon', 'tendinopathy', 'injury', 'rehabilitation', 'high-yield'] },
          { q: 'How do you program exercise to promote tendon health and prevent tendinopathy?', a: 'Load management: progressive overload with adequate recovery time. Avoid large acute spikes in training volume (>10% weekly increase rule as a guideline). Heavy slow resistance (HSR): 3-4 sec eccentric phase, moderate loads, 3×15 — most evidence-based for tendinopathy rehabilitation. Isometric exercises: 45-60 sec holds at 70-80% MVC — immediate pain relief and neural inhibition in symptomatic tendons. Plyometrics last: high-speed tendon loading should follow base of HSR adaptation (12-16 weeks of progressive loading first). Warm-up: increases tendon viscoelasticity and reduces injury risk.', tags: ['tendon', 'programming', 'injury-prevention', 'rehabilitation', 'high-yield'] },
          { q: 'What is reactive strength and how is the Reactive Strength Index (RSI) calculated?', a: 'Reactive strength: ability to rapidly switch from eccentric to concentric contraction (amortization phase) — key quality for sprinting, jumping, and direction change. RSI = Jump height ÷ Ground contact time. High RSI = stiff, explosive spring-like tendon-muscle system. Measured via drop jump on force plate or contact mat. Normative values: team sport athletes ~1.0-1.8; elite sprinters and jumpers >2.0. RSI-modified (RSImod) = jump height ÷ time to takeoff (from countermovement). Training: depth jumps, hurdle hops, bounding — with short ground contact cues ("hot coals").', tags: ['reactive-strength', 'rsi', 'plyometrics', 'tendon', 'power', 'high-yield'] },
        ]
      },
      {
        id: 'sprint-mechanics',
        title: 'Sprint Mechanics & Speed Development',
        cards: [
          { q: 'What are the key mechanical differences between acceleration and maximum velocity sprinting?', a: 'Acceleration phase (0-30m): forward body lean (45° at start, decreasing); force application primarily horizontal; longer ground contact times; step length increasing, frequency increasing; rear foot strike. Maximum velocity phase (30-60m+): upright posture; force application primarily vertical (stiffness model); very short ground contact times (0.08-0.10 sec); step frequency at peak; forefoot contact under center of mass. Coaching focus: acceleration — "push back"; maximum velocity — "pull through" with hip flexion, minimize ground contact time.', tags: ['sprint', 'acceleration', 'max-velocity', 'mechanics', 'high-yield'] },
          { q: 'What is the force-velocity relationship in sprinting and how does it inform training?', a: 'Horizontal force applied to ground = acceleration. Vertical force = maintenance of velocity against gravity. At maximal velocity, horizontal force production is near zero — maintaining velocity requires minimizing energy dissipation (stiffness, minimal braking force). Force-velocity profile: some athletes are force-deficient (can sprint but limited by power output at high speeds → need strength training); some are velocity-deficient (strong but not fast → need speed-specific training). Assess with sprint force-velocity profiling (Samozino method): measure split times across distance to compute force-velocity profile.', tags: ['sprint', 'force-velocity', 'programming', 'high-yield'] },
          { q: 'What are the primary training methods for each phase of sprint development?', a: 'Acceleration (0-10m): sled towing (10-30% body mass), hill sprints, resistance band sprints, broad jumps, pro-agility. Max velocity (30-60m): flying sprints (30m fly), wicket runs (spacing for stride frequency), resisted overspeed, high-knee drills. Speed endurance (glycolytic): 60-150m repeats with full recovery (1:20+ work:rest). Repeated sprint ability: 5-10×20m with 20-30 sec rest — sport-specific. Mechanical correction: A-skips (hip flexion), B-skips (pawing action), ankling drills. Always sprint before lifting when combining in same session.', tags: ['sprint', 'training-methods', 'programming', 'high-yield'] },
        ]
      },
    ]
  },
...
function makeChoices(card, allCards) {
  // Pick distractors from same phase first for plausibility, fill rest from anywhere
  const samePhase = allCards.filter(c => c.cardId !== card.cardId && c.phaseId === card.phaseId);
  const other = allCards.filter(c => c.cardId !== card.cardId && c.phaseId !== card.phaseId);
  const pool = [...samePhase.sort(() => Math.random() - 0.5), ...other.sort(() => Math.random() - 0.5)];
  const distractors = pool.slice(0, 3).map(c => c.a);
